...
def subplots(): #function to plot 6 subplots showing the number of deaths for different regions 
    fig, axs = plt.subplots(2, 3, figsize=(18, 8))
    for i in range(n): #repeating for loop n times to cover all the regions
        plt.subplot(2,3,i+1)
        ...
        years=[]
        for j in range(22): #repeating the loop 22 times to get the year values present in data
...
def top5_piechart(): #function to create pie plot for the top 8 countries with highest number of HIV related deaths
    ...
    labels=[]
    for j in range(8): #repeating loop to get the country names inorder to create labels
...
def multiline(): #function to plot with multiple lines to show number of HIV related deaths in various regions
    for i in range(n):
        #create a new dataframe for all data of a single region
        group3= new_data.loc[(new_data['ParentLocation']== regions[i])]
        group4= group3.groupby(["Period"]).sum()
        years=[]
        for j in range(22): #loop to get the years from 2000 to 2021 for plotting the graph
...
def bar_graph(): #function to plot the bar graph to show the total number of HIV related deaths over the years
...
#reading the csv file into mydata
